...
import { Activity, CheckCircle2, Copy, Download, RefreshCw, XCircle } from 'lucide-react';
import { useProfile } from '../utils/use-profile';
import { mtConnect, mtDisconnect, mtStatus, mtSyncNow, type MtAccountType, type MtPlatform } from '../../lib/mtBridge';
...
const SUPABASE_ORIGIN = (() => {
  ...
  if (!url) return 'https://<project-ref>.supabase.co';
  try {
...
export function MTConnectionDialog({ open, onOpenChange }: MTConnectionDialogProps) {
  ...
  const handleSyncNow = async () => {
    ...
    try {
      const status = await mtSyncNow();

      setIsConnected(Boolean(status.connected));
      setSyncKey(status.syncKey ?? null);
      setSyncUrl(status.syncUrl ?? null);
      setConnectedPlatform(status.connection?.platform ?? null);
      setConnectedAt(status.connection?.connectedAt ?? null);

      const nextLastSyncAt = status.connection?.lastSyncAt ?? status.refreshedAt ?? null;
      setLastSyncAt(nextLastSyncAt);

      const saved = parseSavedConnection(localStorage.getItem(MT_CONNECTION_STORAGE_KEY));
      if (saved?.platform && saved.server && saved.account) {
        const updated: SavedMtConnection = {
          method: 'connector',
          ...
          lastSyncAt: nextLastSyncAt ?? undefined,
        };
        localStorage.setItem(MT_CONNECTION_STORAGE_KEY, JSON.stringify(updated));
      }

      window.dispatchEvent(new Event('mt-connection-changed'));
      toast.success('Synced');
    } catch (error) {
      console.error('MT sync now error', error);
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-2xl p-0 overflow-hidden">
        <div className="max-h-[80vh] overflow-y-auto p-6 pr-12">
          <DialogHeader>
            ...
            <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-3">
              ...
              {isConnected && (
                <div className="flex items-center gap-2 justify-end">
                  <Button variant="outline" onClick={() => void handleSyncNow()} disabled={isSyncingNow}>
                    {isSyncingNow ? (
                      <>
            ...
            <div className="space-y-4">
                ...
                <div className="mt-4 space-y-3">
                  <div className="space-y-1">
                    ...
                    <div className="flex items-center gap-2">
                      <Input readOnly value={syncUrl} className="font-mono text-xs" />
                      <Button
                        type="button"
                        variant="outline"
                        size="icon"
                  ...
                  <div className="space-y-1">
                    ...
                    <div className="flex items-center gap-2">
                      <Input readOnly value={syncKey} className="font-mono text-xs" />
                      <Button
                        type="button"
                        variant="outline"
                        size="icon"
                  ...
                  <div className="flex flex-col sm:flex-row sm:items-center gap-2 pt-2">
                    <Button asChild>
                      <a
                        href="/downloads/TJ_MT5_Connector.ex5"
                        download
                        target="_blank"
                        rel="noreferrer"
                        aria-label="Download TJ MT5 Connector"
                      >
                  ...
                  <p className="text-xs text-muted-foreground">
                    If your browser blocks the download, right click → open in new tab.
                  </p>
                </div>
              </Card>
              ...
              <div className="bg-muted/30 border rounded-lg p-4 text-sm">
                ...
                <ol className="list-decimal list-inside space-y-2 text-muted-foreground">
                  ...
                  <li>
                    Tools → Options → Expert Advisors: enable algorithmic trading and enable WebRequest for listed URLs.
                    Add <span className="font-mono">{SUPABASE_ORIGIN}</span>.
                  </li>
                  <li>Drag the EA onto any chart (any symbol/timeframe).</li>
                  <li>
                    In EA Inputs, paste your <span className="font-medium">Sync URL</span> and{' '}
                    <span className="font-medium">Sync Key</span> from TJ.
                  </li>
                  <li>Keep MetaTrader running; closed trades will sync automatically.</li>
                </ol>
                <p className="text-xs text-muted-foreground mt-3">
                  Tip: Only closed trades should be sent. Keep your Sync Key private.
                </p>
              </div>

              {import.meta.env.DEV && (
                <div className="text-xs text-muted-foreground">
                  Developer: delete test trade (SQL in <span className="font-mono">docs/dev-delete-test-trade.md</span>)
                </div>
              )}
            </div>
          )}
